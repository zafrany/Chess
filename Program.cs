class Game
{
    ...
    public void startGame()
    {
        ...
        while (this.winner == 0 && !draw)
        {
            ...
            if (command != "ff" && !draw)      
                makeMove(locations[0], locations[1]);
        }
        if (!draw)
    ...
    public void makeMove(int startingLocation, int endLocation)
    {
    ...
    {
        int repetitions;
        string[] gameLog = this.board.getGameStates();
        ...
        if (board.deadState() || board.returnMoveCount() == 50 )
            draw = true;
    }
    public void checkForMate(int player)
    {
        int[] legalMovesArray;
        for(int i = 0; i < 8; i++)
        {
    ...
    void doPromotion(int location, int color)
    {
        ...
        while (!legalInput)
        {
            ...
            else
            {
                ...
                {
                    case "Q":
                        board.setPieceAtLocation(location, new Queen("Q", color, location, this.board) , false);
                        legalInput = true;
                        break;
                    case "N":
                        board.setPieceAtLocation(location, new Knight("N", color, location, this.board), false);
                        legalInput = true;
                        break;
                    case "R":
                        board.setPieceAtLocation(location, new Rook("R", color, location, this.board), false);
                        legalInput = true;
                        break;
                    case "B":
                        board.setPieceAtLocation(location, new Bishop("B", color, location, this.board), false);
                        legalInput = true;
                        break;
                    default:
                        Console.WriteLine("Illegal input, type only 1 letter to represent desired piece.");
...
class ChessBoard
{
    ...
    int[] pieceCount = { 0, 0 };  //[0] = black, [1] = white
            ...
            for (int j = 0; j < 8; j++, location ++)
            {
                ...
                if (color != 0)
                {
                    if (i < 1 || i > 6)
                    {
                        if (j == 0 || j == 7)
                        {
                            chessBoard[i, j] = new Rook("R", color, location, this);
                        }

                        else if (j == 1 || j == 6)
                        {
                            chessBoard[i, j] = new Knight("N", color, location, this);
                        }

                        else if (j == 2 || j == 5)
                        {
                            chessBoard[i, j] = new Bishop("B", color, location, this);
                        }

                        else if (j == 3)
                        {
                            chessBoard[i, j] = new Queen("Q", color, location, this);
                        }

                        else if (j == 4)
                        {
                            chessBoard[i, j] = new King("K", color, location, this);
                        }
                    }

                    else 
                        chessBoard[i, j] = new Pawn("P", color, location, this);
                }
    ...
    public void updateGameState(int flag) //flag 0 = dont clear array and add new stat, flag 1 = clear array before adding new state
    {
    ...
    public void setBoard(string gameString)
    {
        ...
        for (int i = 0; i < 64; i++)
        {
            ...
            {
                case 'R':
                    this.chessBoard[i / 8 , i % 8] = new Rook("R", color, i, this);
                    break;
                case 'N':
                    this.chessBoard[i / 8 , i % 8] = new Knight("N", color, i, this);
                    break;
                case 'B':
                    this.chessBoard[i / 8 , i % 8] = new Bishop("B", color, i, this);
                    break;
                case 'Q':
                    this.chessBoard[i / 8, i % 8] = new Queen("Q", color, i, this);
                    break;
                case 'K':
                    this.chessBoard[i / 8 , i % 8] = new King("K", color, i, this);
                    this.setKingLocation(color, i);
                    break;
                case 'P':
                    this.chessBoard[i / 8 , i % 8] = new Pawn("P", color, i, this);
...
abstract class ChessPiece
{
    ...
    public ChessPiece(string type, int color, int location, int[] possibleMoveDirections, ChessBoard board)
    {
        this.type = type;
        this.color = color;
        ...
        if (this.legalMoves.Contains(destination))
        {
            ...
            if (this.type == "P")
            {
                ...
                if (Math.Abs(destination - location) == 16)
                {
                    ...
                    if (west > 0 && board.pieceAtLocation(destination - 1).getType() == "P" && board.pieceAtLocation(destination - 1).getColor() != this.getColor())
                        ((Pawn)board.pieceAtLocation(destination - 1)).setEnPassant(destination);
                    
                    if (east > 0 && board.pieceAtLocation(destination + 1).getType() == "P" && board.pieceAtLocation(destination + 1).getColor() != this.getColor())
                        ((Pawn)board.pieceAtLocation(destination + 1)).setEnPassant(destination);
                }
            }

            //remove all current en passant options as they are no longer possible
            for (int i = 0; i < 8; i++)
                for (int j = 0; j < 8; j++)
            ...
            //update game stats to help check for 3-fold repetition
            board.updateGameState(flag);
            board.updateMoveCount(flag);
...
{
    public EmptySquare(int location, ChessBoard board) : base("E", 0, location, new int[] {0}, board){ }
...
{
    public Rook(string type, int color, int location, ChessBoard board) : base(type, color, location, new int[] {-8, -1, 1, 8}, board) { }
    public override int[] legalMove(int location)
    {
        ...
        //Console.WriteLine(legalMovesString);
...
{
    public Knight(string type, int color, int location, ChessBoard board) : base(type, color, location, new int[] { -17, -15, -10, -6, 6, 10, 15, 17 }, board) { }
    public override int[] legalMove(int location)
    {
        string legalMovesString = "";
        int[] possibleDirections = this.getPossibleMoveDirections();
        ...
        //Console.WriteLine("knight location = " + location);
        //Console.WriteLine("{0} knight legal moves = " + legalMovesString, this.getColor()==-1?"Black":"White");
        return legalMovesLocations;
    }
}
...
{
    public Bishop(string type, int color, int location, ChessBoard board) : base(type, color, location, new int[] { -9, -7, 7, 9 }, board) { }
    public override int[] legalMove(int location)
    {
        int[] possibleDirections = this.getPossibleMoveDirections();
        int color = this.getColor();
...
{
    public Queen(string type, int color, int location, ChessBoard board) : base(type, color, location, new int[] { -9, -8, -7, -1, 1, 7, 8, 9 }, board) { }
    public override int[] legalMove(int location)
    {
        int[] possibleDirections = this.getPossibleMoveDirections();
        int color = this.getColor();
...
class King : ChessPiece
{
    ...
    public King(string type, int color, int location, ChessBoard board) : base(type, color, location, new int[] { -9, -8, -7, -1, 1, 7, 8, 9 }, board) 
    {
...
class Pawn : ChessPiece
{
    ...
    public Pawn(string type, int color, int location, ChessBoard board) : base(type, color, location, new int[] { 8, 16 }, board) 
    {
...
class Program
{
    public static void Main()
    {
        ...
        game.GetBoard().printBoardSymbols();
        game.startGame();
    }
}